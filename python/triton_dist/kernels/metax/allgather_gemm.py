...
# TODO: MACA DIST: support compiler get_module_map
from triton.language.extra.maca import libshmem_device as libshmem_device
from triton.language.extra.maca.language_extra import tid, __syncthreads
import pymxshmem
import copy
import time
...
from triton_dist.kernels.metax.common_ops import barrier_all
...
def cp_engine_producer_all_gather_full_mesh_pull(
    ...
    with torch.cuda.stream(ag_stream):
        ...
        for src_rank in rank_orders:
            if src_rank == rank:
                continue
            #dst = remote_tensor_buffers[src_rank][rank * M_per_rank:(rank + 1) * M_per_rank, :]
            #src = remote_tensor_buffers[rank][rank * M_per_rank:(rank + 1) * M_per_rank, :]
            #dst.copy_(src)

            #one_t = torch.ones((1), device="cuda")
            #barrier_buffers[src_rank][rank] = one_t
            # TODO(MACA): use multi cp engines for multi chunks.
            rank_base = src_rank * M_per_rank
            for chunk_idx in range(num_chunks_per_rank):    
                chunk_offset = chunk_idx * M_per_chunk
                dst = remote_tensor_buffers[rank][rank_base + chunk_offset:rank_base + chunk_offset + M_per_chunk, :]
                src = remote_tensor_buffers[src_rank][rank_base + chunk_offset:rank_base + chunk_offset + M_per_chunk, :]
                dst.copy_(src)

                barrier_buffers[rank][src_rank * num_chunks_per_rank + chunk_idx] = signal_t
            #FIXME(MACA):
    ...
    with torch.cuda.stream(ag_stream):
        ...
        MACA_CHECK(err)
            #     dst.copy_(src)
                ...
                # barrier
                # memcpy D2D kernel:
                # barrier_buffers[src_rank][rank * num_chunks_per_rank + chunk_idx] = signal_t
                
                # cuStreamWriteValue32:
                # (err, ) = cuda.cuStreamWriteValue32(
                #     ag_stream.cuda_stream,
                #     barrier_buffers[src_rank][rank * num_chunks_per_rank + chunk_idx].data_ptr(),
                #     signal_t,
                #     cuda.CUstreamWriteValue_flags.CU_STREAM_WRITE_VALUE_DEFAULT,
                # )
                # CUDA_CHECK(err)
...
def cp_engine_producer_all_gather_numa_node_push(
    ...
    with torch.cuda.stream(internode_ag_stream):
        ...
        #   src = local_tensor
        #   dst = remote_tensor_buffers[p2p_send_rank][rank * M_per_rank]
        #   dst.copy_(src)
        #   barrier_buffers[p2p_send_rank][rank] = 1
    ...
    with torch.cuda.stream(intranode_ag_stream):
        ...
        for chunk_idx in range(num_chunks_per_rank):
            ...
            for rank_idx, src_local_rank in enumerate(rank_orders):
                ...
                # intra_engine[req_idx] = macart.mcParallelCopyEngine.ParallelCopyEngineDefault
    ...
    if not cpengine_dispatch:
        ...
        with torch.cuda.stream(internode_ag_stream):
            ...
            if n_numa_nodes > 1:
                ...
                #with torch.cuda.stream(numanode_ag_stream):
                ...
                for chunk_idx in range(num_chunks_per_rank):
                    ...
                    for i in range(1, n_numa_nodes):
                        ...
                        #dst = ag_buffer[p2p_send_rank][rank_base + chunk_offset:rank_base + chunk_offset + M_per_chunk, :]
                        dst = ag_buffer[p2p_send_rank].data_ptr() + (rank_base + chunk_offset) * N * local_tensor.element_size()
                        #src = local_tensor[chunk_offset:chunk_offset + M_per_chunk, :]
                        #src = ag_buffer[local_rank][rank_base + chunk_offset:rank_base + chunk_offset + M_per_chunk, :]
                        ...
                        #write_flag[req_idx] = signal_buffer[p2p_send_rank][rank * num_chunks_per_rank + chunk_idx].data_ptr()
                    ...
                    if node_id != node_rank:
                        ...
                        for chunk_idx in range(num_chunks_per_rank):
                            ...
                            for i in range(1, n_numa_nodes):
                                ...
                                engine[req_idx] = macart.mcParallelCopyEngine.ParallelCopyEngine0
                                count[req_idx] = M_per_chunk * N * local_tensor.element_size()
                                #wait_flag[req_idx] = signal_buffer[local_rank][rank_idx * num_chunks_per_rank + chunk_idx].data_ptr()
        ...
        with torch.cuda.stream(intranode_ag_stream):
            ...
            for chunk_idx in range(num_chunks_per_rank):
                for i in range(1, numa_world_size):
                    ...
                    #src_ptr = local_tensor.data_ptr() + segment * local_tensor.element_size()
                    ...
                    #write_flag[idx] = signal_buffer[local_dst_rank][rank * num_chunks_per_rank + chunk_idx].data_ptr()
            ...
            for i in range(1, n_nodes * n_numa_nodes):
                ...
                for chunk_idx in range(num_chunks_per_rank):
                    ...
                    for j in range(1, numa_world_size):
                        ...
                        engine[idx] = engine_list[idx % len(engine_list)]
                        count[idx] = M_per_chunk * N * local_tensor.element_size()
                        #write_flag[idx] = signal_buffer[local_dst_rank][recv_rank* num_chunks_per_rank + chunk_idx].data_ptr()
                        ...
                        write_value[idx] = signal_t
                        #wait_flag[idx] = signal_buffer[local_rank][recv_rank * num_chunks_per_rank + chunk_idx].data_ptr()
...
def ag_gemm_inter_node_op(a, b, c, rank, num_ranks, fullmesh_world_size, num_chunks_per_rank, workspace_tensors, barrier_tensors, comm_buf,
    ...
    #numanode_ag_stream = torch.cuda.Stream() if numa_nodes_num > 1 else None
    #internode_ag_stream.wait_stream(current_stream)
...
def ag_gemm_intra_node(a, b, ctx=None, rank=None, num_ranks=None, use_tma=True):
    ...
    # Just reuse mxshmem barrier, they should produce similar performance

    use_pull = False # default cp_engine_allgather_push
    ctx.barrier_tensors[ctx.rank].fill_(0)
    current_stream = torch.cuda.current_stream()
    # if not use_pull:
    #     # to make sure barrier fill 0 on all devices
    #     pymxshmem.mxshmem_barrier_all_on_stream(current_stream.cuda_stream)
    # ctx.workspace_tensors[ctx.rank][ctx.rank * M_per_rank:(ctx.rank + 1) * M_per_rank, :].copy_(a)

    # signal_t = torch.ones((1), device="cuda", dtype=torch.int32)
    signal_t = 1
    # for i in range(ctx.num_chunks_per_rank):
    #     ctx.barrier_tensors[ctx.rank][ctx.rank * ctx.num_chunks_per_rank + i] = signal_t
    ...
    if use_tma:
        ag_gemm_intra_node_persistent_op(a, b, C, ctx.rank, ctx.num_ranks, ctx.workspace_tensors, ctx.barrier_tensors,
                                         ...
                                         gemm_stream=ctx.gemm_stream, serial=ctx.serial, autotune=ctx.autotune, use_pull=use_pull, signal_t=signal_t)
    else:
    ...
    # sync to make sure local memory will not be free before trans.
    pymxshmem.mxshmem_barrier_all_on_stream(current_stream.cuda_stream)
...
def create_ag_gemm_inter_node_context(tensor_A, tensor_B, rank, num_ranks, num_local_ranks=8, max_M=2**14,
    ...
    #internode_workspaces = pymxshmem.mxshmem_create_tensor_list_intra_node([M_per_rank*num_ranks, K], dtype)
    barriers = pymxshmem.mxshmem_create_tensor_list_intra_node([num_ranks * num_chunks_per_rank], torch.uint64)
    #inter_barriers = pymxshmem.mxshmem_create_tensor_list_intra_node([num_ranks * num_chunks_per_rank], torch.uint64)
    #symm_ag_a = symm_mem.empty([M_per_rank*num_ranks, K], dtype=dtype, device=tensor_A.device)
    #symm_mem_hdl = symm_mem.rendezvous(symm_ag_a, group=dist.group.WORLD)
    #workspaces = []
    ...
    #inter_barriers[local_rank].fill_(0)
    current_stream = torch.cuda.current_stream()
    pymxshmem.mxshmem_barrier_all_on_stream(current_stream.cuda_stream)
    torch.cuda.synchronize()
...
def ag_gemm_inter_node(a, b, ctx=None, rank=None, num_ranks=None, local_world_size=8, signal_target=1, use_tma=True):
    ...
    #ctx.inter_barrier_tensors[local_rank].fill_(0)
    current_stream = torch.cuda.current_stream()
    pymxshmem.mxshmem_barrier_all_on_stream(current_stream.cuda_stream)
    ctx.workspace_tensors[local_rank][ctx.rank * M_per_rank:(ctx.rank + 1) * M_per_rank, :].copy_(a)
    # ctx.internode_workspaces[local_rank][ctx.rank * M_per_rank:(ctx.rank + 1) * M_per_rank, :].copy_(a)
    #signal_t = torch.ones((signal_target), device="cuda")
    #for i in range(ctx.num_chunks_per_rank):
    #    ctx.barrier_tensors[local_rank][ctx.rank * ctx.num_chunks_per_rank + i] = signal_t
    #    # signal_t = torch.ones((1), device="cuda", dtype=torch.int32)
    signal_t = 1
    # for i in range(ctx.num_chunks_per_rank):
    #     ctx.barrier_tensors[ctx.rank][ctx.rank * ctx.num_chunks_per_rank + i] = signal_t
    ctx.barrier_tensors[local_rank][ctx.rank * ctx.num_chunks_per_rank:(ctx.rank+1) * ctx.num_chunks_per_rank].fill_(signal_t)
    #ctx.inter_barrier_tensors[local_rank][ctx.rank * ctx.num_chunks_per_rank:(ctx.rank+1) * ctx.num_chunks_per_rank].fill_(signal_t)
    # flush L2
    pymxshmem.flush_l2c(current_stream.cuda_stream)
    # make sure local copy and flush_l2c finished on all devices
    pymxshmem.mxshmem_barrier_all_on_stream(current_stream.cuda_stream)    
    ...
    if use_tma:
        ag_gemm_inter_node_persistent_op(a, b, C, ctx.rank, ctx.num_ranks, ctx.workspace_tensors, ctx.barrier_tensors,
                                         ...
                                         copy_engine_dispatch=True, signal_t=signal_t)
    else:
