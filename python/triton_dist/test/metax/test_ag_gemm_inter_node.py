...
def init_mxshmem_by_uniqueid(group: torch.distributed.ProcessGroup):
    rank, nranks = group.rank(), group.size()
    if rank == 0:
        unique_id: bytes = pymxshmem.mxshmemx_get_uniqueid()
    ...
    pymxshmem.mxshmemx_init_attr_with_uniqueid(rank, nranks, unique_id)
...
def perf_test(M, config):
    ...
    def triton_func():
        return ag_gemm_inter_node(A, B, ctx=ctx, local_world_size=LOCAL_WORLD_SIZE, use_tma=False)
...
if __name__ == "__main__":
    ...
    init_mxshmem_by_uniqueid(TP_GROUP)
    ...
    if args.gemm_only:
        ...
        from third_party.distributed.distributed.kernels.metax.allgather_gemm import inter_node_allgather, local_copy_and_barrier_all
        for _, value in configs.items():
